...
export const API_DATA: Record<string, ApiProp[]> = {
  ...
  'line-layer': [
    ...
    { name: 'onMouseOver', type: '(e: LineLayerEvent) => void', required: false, description: '鼠标移入要素（需 enablePicked），2.0.2 新增' },
    { name: 'onMouseOut', type: '(e: LineLayerEvent) => void', required: false, description: '鼠标移出要素（需 enablePicked），2.0.2 新增' },
    { name: 'onMouseMove', type: '(e: LineLayerEvent) => void', required: false, description: '鼠标在要素上移动（需 enablePicked），2.0.2 新增' },
    { name: 'onReady', type: '(layer: any) => void', required: false, description: '图层挂载后回调，拿到原生 LineLayer 实例做命令式操作（如 updateState/clearState 配合 feature-state 高亮），2.0.2 新增' },
  ],
