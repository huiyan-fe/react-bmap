...
export const API_DATA: Record<string, ApiProp[]> = {
  ...
  map: [
    ...
    { name: 'enableIconInfoWindow', type: 'boolean', required: false, description: '开启点击底图标注后自动展示信息窗口，同时会开启底图标注点击；3.0 下等价于 enableMapClick，2.0.2 新增' },
  ...
  'fill-layer': [
    ...
    { name: 'onDblClick', type: '(e: FillLayerEvent) => void', required: false, description: '双击要素（需 enablePicked），2.0.2 新增' },
    { name: 'onRightClick', type: '(e: FillLayerEvent) => void', required: false, description: '右键点击要素（需 enablePicked），2.0.2 新增' },
    { name: 'onMouseMove', type: '(e: FillLayerEvent) => void', required: false, description: '鼠标在要素上移动（需 enablePicked）；SDK 不派发 mouseover/mouseout，2.0.2 新增' },
    { name: 'enableChangeSelectIndexByPick', type: 'boolean', required: false, description: '拾取时是否自动切换 selectedIndex；配合 feature-state 高亮时通常设 false，2.0.2 新增' },
    { name: 'setDataParams', type: 'object', required: false, description: 'setData 第二参（如 { changeCenter: true } 自动定位），2.0.2 新增' },
  ...
  'point-icon-layer': [
    ...
    { name: 'onDblClick', type: '(e: PointIconLayerEvent) => void', required: false, description: '双击要素（需 enablePicked），2.0.2 新增' },
    { name: 'onRightClick', type: '(e: PointIconLayerEvent) => void', required: false, description: '右键点击要素（需 enablePicked），2.0.2 新增' },
    { name: 'onMouseMove', type: '(e: PointIconLayerEvent) => void', required: false, description: '鼠标在要素上移动（需 enablePicked）；SDK 不派发 mouseover/mouseout，2.0.2 新增' },
  ],
  'point-shape-layer': [
  ...
  'line-layer': [
    { name: 'style', type: 'LineLayerStyle', required: false, description: '线样式（strokeColor/strokeWeight/strokeStyle/dashArray/borderColor/borderWeight 等）；strokeColor/strokeTextureUrl/strokeTextureWidth 等支持数据驱动表达式（如 ["match", ["get","name"], "a", "#f00", "#00f"]）；纹理贴图用 strokeTextureUrl + sequence（沿线重复）+ marginLength（间隔）。LineLayer 自 2.0.2 起支持 data 渲染，style 变化时运行时 setStyleOptions 更新、不重建图层' },
    ...layerData,
    ...layerCommon,
    { name: 'enablePicked', type: 'boolean', required: false, description: '是否可拾取（响应鼠标事件），click 命中要素时 e.value.dataIndex 为要素索引' },
    { name: 'enableChangeSelectIndexByPick', type: 'boolean', required: false, description: '拾取时是否自动切换 selectedIndex；配合 feature-state 高亮时通常设 false，自己控制状态，2.0.2 新增' },
    { name: 'onClick', type: '(e: LineLayerEvent) => void', required: false, description: '点击要素（需 enablePicked）；e.value.dataIndex=-1 表示点空白，2.0.2 新增' },
    { name: 'onDblClick', type: '(e: LineLayerEvent) => void', required: false, description: '双击要素（需 enablePicked），2.0.2 新增' },
    { name: 'onRightClick', type: '(e: LineLayerEvent) => void', required: false, description: '右键点击要素（需 enablePicked），2.0.2 新增' },
    { name: 'onMouseMove', type: '(e: LineLayerEvent) => void', required: false, description: '鼠标在要素上移动（需 enablePicked）；SDK 不派发 mouseover/mouseout，2.0.2 新增' },
    { name: 'setDataParams', type: 'object', required: false, description: 'setData 第二参（如 { changeCenter: true } 自动定位），2.0.2 新增' },
    { name: 'onReady', type: '(layer: any) => void', required: false, description: '图层挂载后回调，拿到原生 LineLayer 实例做命令式操作（updateState/clearState/traceControl 等），2.0.2 新增' },
  ],
