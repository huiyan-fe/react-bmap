...
const routeReturn: ApiProp[] = [
  ...
  { name: '返回值.setPolylineStyle()', type: '(style: Record<string, unknown>) => void', required: false, description: '运行时设置路线折线样式（strokeColor / strokeWeight / strokeOpacity 等），2.0.4 新增' },
...
export const API_DATA: Record<string, ApiProp[]> = {
  ...
  'local-search': [
    ...
    { name: 'onInfoHtmlSet', type: '(poi: unknown, html: HTMLElement) => void', required: false, description: '信息窗内容设置回调' },
    { name: 'onResultsHtmlSet', type: '(container: HTMLElement) => void', required: false, description: '结果面板渲染回调' },
    { name: 'onPolylinesSet', type: '(polylines: unknown[]) => void', required: false, description: '折线添加完成回调（对应 setPolylinesSetCallback），2.0.4 新增' },
    ...
    { name: '返回值.select()', type: '(index: number) => void', required: false, description: '选中/高亮某条结果并打开其信息窗，2.0.4 新增' },
    { name: '返回值.clearSelected()', type: '() => void', required: false, description: '取消选中：关信息窗 + 清结果列表选中态（GL 有效，非空实现），2.0.4 新增' },
    { name: '返回值.setLocation()', type: '(location: string | Point | MapHandle) => void', required: false, description: '运行时设置检索城市/区域，2.0.4 新增' },
    { name: '返回值.enableAutoViewport() / disableAutoViewport()', type: '() => void', required: false, description: '开启/关闭检索后自动调整视野，2.0.4 新增' },
    { name: '返回值.enableFirstResultSelection() / disableFirstResultSelection()', type: '() => void', required: false, description: '开启/关闭「自动选中第一个结果」，2.0.4 新增' },
    { name: '返回值.setPageCapacity()', type: '(n: number) => void', required: false, description: '运行时设置每页结果数（1-100），2.0.4 新增' },
    { name: '返回值.getStatus()', type: '() => number | undefined', required: false, description: '最近一次检索的状态码（BMAP_STATUS_*），2.0.4 新增' },
  ...
  geocoder: [
    ...
    { name: '返回值.setOptions()', type: '(options: Record<string, unknown>) => void', required: false, description: '设置地理编码选项（如 language 语言），对应 SDK setOptions，2.0.4 新增' },
  ...
  'bus-line-search': [
    ...
    { name: '返回值.clearResults()', type: '() => void', required: false, description: '清空 hook 的 data 状态；SDK 无原生 clear，地图上已渲染的线路不受影响' },
    { name: '返回值.enableAutoViewport() / disableAutoViewport()', type: '() => void', required: false, description: '开启/关闭检索后自动调整视野，2.0.4 新增' },
    { name: '返回值.setLocation()', type: '(location: string | MapHandle) => void', required: false, description: '运行时设置检索城市/区域，2.0.4 新增' },
    { name: '返回值.getStatus()', type: '() => number | undefined', required: false, description: '最近一次检索的状态码，2.0.4 新增' },
  ...
  autocomplete: [
    ...
    { name: '返回值.setInputValue()', type: '(value: string) => void', required: false, description: '设置绑定输入框的值，2.0.4 新增' },
    { name: '返回值.setTypes()', type: '(types: string[]) => void', required: false, description: '运行时设置返回结果类型限定，2.0.4 新增' },
    { name: '返回值.setLocation()', type: '(location: unknown) => void', required: false, description: '运行时设置检索城市/区域，2.0.4 新增' },
    { name: '返回值.getStatus()', type: '() => number | undefined', required: false, description: '最近一次的状态码，2.0.4 新增' },
  ...
  boundary: [
    ...
    { name: '返回值.parsebdStr()', type: '(boundaryStr: string) => unknown', required: false, description: '把边界字符串（"lng,lat;lng,lat;…"）解析成点集，2.0.4 新增' },
  ...
  'panorama-service': [
    ...
    { name: '返回值.getPanoramaByLocation()', type: '(point: Point, radius?: number) => void', required: false, description: '按坐标查询附近全景，radius 为搜索半径（米）；radius 可省略，走 SDK 默认约 50 米（2.0.2 起）' },
    { name: '返回值.getPanoramaByPOIId()', type: '(poiId: string) => void', required: false, description: '按 POI id 查询全景（部分版本 SDK 支持，运行时不支持时置 error），2.0.2 新增' },
  ...
  panorama: [
    ...
    { name: 'zoom', type: 'number', required: false, description: '缩放级别 → setZoom，2.0.2 新增' },
    { name: 'options', type: 'PanoramaOptions', required: false, description: '全景配置：navigationControl / linksControl / indoorSceneSwitchControl / albumsControl(Options)，构造时应用并支持运行时 setOptions，2.0.2 新增' },
    { name: 'visible', type: 'boolean', required: false, description: '显隐：true→show() / false→hide()，2.0.2 新增' },
    { name: 'enableScrollWheelZoom', type: 'boolean', required: false, description: '鼠标滚轮缩放开关，2.0.2 新增' },
    { name: 'poiType', type: 'PanoramaPOIType', required: false, description: '外景可见 POI 类型（BMAP_PANORAMA_POI_*）→ setPanoramaPOIType，2.0.2 新增' },
    { name: 'ref', type: 'Ref<PanoramaRef>', required: false, description: '命令式句柄：getPosition/getPov/getZoom/getId/getLinks/getVisible/getSceneType/capture/setXxx/show/hide/clearOverlays/raw，2.0.2 新增' },
    { name: 'style', type: 'CSSProperties', required: false, description: '容器样式' },
    { name: 'className', type: 'string', required: false, description: '自定义类名' },
    { name: 'onPositionChange', type: '(point: Point) => void', required: false, description: '位置变化回调（position_changed），回调参数是新的位置点' },
    { name: 'onPovChange', type: '() => void', required: false, description: '视角变化回调（pov_changed），无回调参数' },
    { name: 'onLinksChange', type: '() => void', required: false, description: '相邻道路数据变化回调（links_changed），2.0.2 新增' },
    { name: 'onZoomChange', type: '(zoom: number) => void', required: false, description: '缩放级别变化回调（zoom_changed），2.0.2 新增' },
    { name: 'onClick', type: '(e: unknown) => void', required: false, description: '单击全景画面（click），2.0.2 新增' },
    { name: 'onDblClick', type: '(e: unknown) => void', required: false, description: '双击全景画面（dblclick），2.0.2 新增' },
    { name: 'onLinkClick', type: '(e: unknown) => void', required: false, description: '单击道路链接（link_click），2.0.2 新增' },
    { name: 'onIdChange', type: '(id: string) => void', required: false, description: '全景 id 变化（id_changed），2.0.2 新增' },
    { name: 'onSceneTypeChange', type: '() => void', required: false, description: '场景类型变化（scene_type_changed），2.0.2 新增' },
    { name: 'onError', type: '(e: unknown) => void', required: false, description: '全景数据加载失败（pano_error），2.0.2 新增' },
    { name: 'onDataLoad', type: '(e: unknown) => void', required: false, description: '全景数据加载完成（dataload），2.0.2 新增' },
    // PanoramaLabel 作为 <Panorama> 子组件使用
    { name: 'PanoramaLabel.position', type: 'Point', required: true, description: '（子组件 <PanoramaLabel>）标注经纬度位置' },
    { name: 'PanoramaLabel.altitude', type: 'number', required: false, description: '距地面高度（米），默认 2；近大远小' },
    { name: 'PanoramaLabel.content', type: 'string', required: false, description: '标注文本内容' },
    { name: 'PanoramaLabel.displayDistance', type: 'boolean', required: false, description: '是否显示标签到当前场景点的距离，默认 true，2.0.2 新增' },
    { name: 'PanoramaLabel.customStyle', type: 'CSSProperties', required: false, description: '自定义标签 css 样式，2.0.2 新增' },
    { name: 'PanoramaLabel.onClick', type: '(e: unknown) => void', required: false, description: '点击标注回调（click 事件），2.0.2 新增' },
  ],
  'place-detail-panel': [
