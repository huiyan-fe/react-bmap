...
describe('CAPABILITY_MATRIX', () => {
  ...
  it('全版本共有能力在两套矩阵里都存在', () => {
    for (const cap of ['Map', 'Map.setCenter', 'Marker', 'NavigationControl', 'LocalSearch', 'Map.enableIconInfoWindow', 'Map.disableIconInfoWindow', 'Map.addControl', 'Map.removeControl', 'Map.setMapStyle', 'Map.setMapStyleV2']) {
  ...
  it('4.0-only 能力仅在 v4 矩阵', () => {
    for (const cap of ['Map.flyTo', 'Marker3D', 'GeoJSONLayer']) {
      expect(CAPABILITY_MATRIX['4.0'].has(cap)).toBe(true);
  ...
  it('3.0-only 能力仅在 v3 矩阵', () => {
    for (const cap of ['Map.enableMapClick', 'CustomLayer', 'PointCollection']) {
      expect(CAPABILITY_MATRIX['3.0'].has(cap)).toBe(true);
