...
export function createV4Driver(rawSDK: any, opts: { unsupportedBehavior: UnsupportedBehavior }): BMapDriver {
  ...
  return {
    ...
    createSymbol: (path, o) => {
      ...
      if (typeof raw?.scale === 'number') ctorOpts.scale = raw.scale;
      if (typeof raw?.rotation === 'number') ctorOpts.rotation = raw.rotation;
      if (typeof raw?.strokeColor === 'string') ctorOpts.strokeColor = raw.strokeColor;
      if (typeof raw?.strokeOpacity === 'number') ctorOpts.strokeOpacity = raw.strokeOpacity;
      if (typeof raw?.strokeWeight === 'number') ctorOpts.strokeWeight = raw.strokeWeight;
      const hasOpts = Object.keys(ctorOpts).length > 0;
      return createOverlayFactory('Symbol', () =>
