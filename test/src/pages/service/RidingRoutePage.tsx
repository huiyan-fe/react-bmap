...
export function RidingRoutePage() {
  const caps = useCapabilities();
  const supported = caps.has('RidingRoute');
  ...
  const { data, loading, error, search, clearResults, enableAutoViewport, disableAutoViewport, getStatus, cancel } = useRidingRoute({
    location: '北京',
  ...
  return (
    <div className="test-page">
      ...
      <div className="test-controls">
        ...
        <section><h3>能力</h3><span className={`cap-tag ${supported ? 'ok' : 'no'}`}>{supported ? 'v4+' : 'v3 ✗'}</span><p className="muted small">@since 4.0。骑行路线规划。search 只接受坐标(Point)。v3 不支持。</p></section>
        ...
        <section><h3>终点 (lng,lat)</h3><input className="full-width" value={end} onChange={e => setEnd(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} /><p className="muted small">默认：西单(116.397,39.908)</p></section>
        ...
        <section><h3>代码示例</h3><pre style={{ fontSize: 10, background: '#f5f5f5', padding: 8, borderRadius: 4, overflow: 'auto' }}>{`const { search, clearResults, enableAutoViewport, getStatus, data, cancel } = useRidingRoute({
  location: '北京',
