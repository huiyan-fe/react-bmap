...
export function WalkingRoutePage() {
  const caps = useCapabilities();
  const supported = caps.has('WalkingRoute');
  const [start, setStart] = useState('116.404,39.915');
  const [end, setEnd] = useState('116.417,39.928');
  ...
  const { data, loading, error, search, clearResults, enableAutoViewport, disableAutoViewport, getStatus, cancel } = useWalkingRoute({
    location: '北京',
  ...
  return (
    <div className="test-page">
      <div className="test-map"><Map defaultCenter={BEIJING} defaultZoom={14} style={{ height: '100%' }}><MapHandleCapture onMap={setMapHandle} /></Map></div>
      <div className="test-controls">
        <h2>useWalkingRoute</h2>
        <section><h3>能力</h3><span className={`cap-tag ${supported ? 'ok' : 'no'}`}>{supported ? 'supported' : 'unsupported'}</span><p className="muted small">全版本共有。步行路线规划。search 只接受坐标(Point)。</p></section>
        ...
        <section><h3>终点 (lng,lat)</h3><input className="full-width" value={end} onChange={e => setEnd(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} /><p className="muted small">默认：北海公园(116.417,39.928)</p></section>
        ...
        <section><h3>代码示例</h3><pre style={{ fontSize: 10, background: '#f5f5f5', padding: 8, borderRadius: 4, overflow: 'auto' }}>{`const { search, clearResults, enableAutoViewport, getStatus, data, cancel } = useWalkingRoute({
  location: '北京',
  renderOptions: { map, panel: domEl, autoViewport: true },
});
search({ lng: 116.404, lat: 39.915 }, { lng: 116.417, lat: 39.928 });`}</pre></section>
      </div>
    </div>
  );
}
